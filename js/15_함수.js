...
/* #btn1 클릭 시 #input1 값을 읽어와 print1() 함수 호출 */
...
document.querySelector("#btn2a").addEventListener("click", function(){
    const arr1 = [10, 20, 30];
    const arr2 = ["사과", "바나나", "복숭아"];

    /* 참조란?
      - 객체 지향 언어에서 배열, 객체 등 object는 값을 여러 개 가지고 있는 자료형인데
        변수는 값을 1개만 저장할 수 있기 때문에 

        object를 메모리 다른 곳에 저장하고 저장된 것의 주소를 변수에 저장해서 
        필요할 때마다 해당 주소를 찾아가 

        원하는 값을 얻어오거나 수정할 수 있게 하는 것

        이 때, 주소를 찾아가는 것을 [참조] 리고 한다 */
    arrayTest(arr1);
    arrayTest(arr2);
    ...
    /* 왜 복숭아가 아니라 딸기가 출력?
        -> arrayTest(arr2) 호출 시
           ...
           arrTest() 함수에서 arr2와 같은 배열을 참조해서 수정
            -> 함수 수행 후 돌아와서도 참조하고 있던 배열의 인덱스 값이 변화되어 있음 */
...
function pow(num, x){
    ...
    let result = 1;  // 곱했을 때 결과에 영행을 주지 않기 위해 1 초기화
    for(let i = 0; i<x; i++){
        result *= num;
    }
...
/* 화살표 함수 */
const arrowList = document.querySelectorAll(".arrow");  // 배열 형태
...
function printObject(othrtFn){
    const obj = othrtFn("홍길동", 20);
