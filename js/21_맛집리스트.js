...
function Restaurant(resName, resAddress, resMenu){
    ...
    // 기능(메서드)
    this.imformation = function(){
        return `${this.resName} / [${this.resMenu}] / ${resAddress}`
    }
}
...
resList.push(new Restaurant("왕가탕후루", "샤인머스켓, 통귤, 딸기", "서울시 서대문구"));
...
/* 조회버튼 클릭 시 #printList의 이전 내용을 모두 삭제하고 resList의 요소를 이용해 li 요소를 만들어 추가 */
...
selectBtn .addEventListener("click", ()=>{
    ...
    for(let i =0; i < resList.length; i++){
        ...
        // 만들어진 li의 내용으로 resList[i]의 imformation() 메서드 반환 값 대입
        li.innerText = resList[i].imformation();
