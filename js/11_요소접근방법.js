...
function classTest() {
    ...
    /*  유사 배열이란?
        배열퍼럼 index, length를 가지고 있으나
        배열 전용 기능(메서드)를 제공하지 않음
    */

    // .cls-test 요소 모두 얻어오기
    // -> HTMLCollection(유사배열) 형태로 얻어옴
...
function nameTest(){
    ...
    for( let i =0 ; i<hobbyList.length; i++) {  // 순차 접근
        ...
        // input요소.checked -> 요소가 체크되어 있으면 true, 아니면 flase
        ...
        // 체크된 요소의 값을 str 변수에 누적 + count 변수 값을 1 중가
  ...
  function cssTest() {
    ...
    */
    ...
    // 첫 번째 자식 div 선택 방법 2
    // -> querySelector()의 !!첫 번째 요소만 선택!!한다는 특징 활용
...
function readValue() {
    ...
    bg.innerHTML += `<p><span>${input.value}</span></p>`;  // 채팅 출력
